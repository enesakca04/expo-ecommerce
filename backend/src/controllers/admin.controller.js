...
export async function createProduct(req,res){
    try {
        ...
        res.status(500).json({message:" ınternal server error"})        
    }
}
...
export async function updateOrderStatus(req,res) {
    try {
        ...
        if(!["pending","shipped","delivered"].includes(status)){
            return res.status(400).json({error: "ınvalis status"})
        }
        ...
        await order.save()
        res.status(200).json({message:"order status update successfully", order})
    } catch (error) {
